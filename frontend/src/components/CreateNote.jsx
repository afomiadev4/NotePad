  ...
  return (
    <div className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-main)] flex transition-colors duration-300">
      ...
      <main className="flex-1 lg:ml-64 p-6 md:p-12">
        <form onSubmit={handleSave} className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-4">
            <div className="bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-[2.5rem] p-8 space-y-8 sticky top-12 shadow-xl">

              {/* Word Count Progress */}
              <div className="p-4 bg-[var(--bg-primary)] rounded-2xl border border-[var(--border-color)]">
                <div className="flex justify-between items-end mb-2">
                  <label className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[0.2em]">Word Count</label>
              ...
              <div className="space-y-3">
                ...
                <div className="relative group">
                  ...
                  >
                    {[ "General", "Life", "Questions", "Fun/Random", "Creative", "Thoughts" ].map(cat => (
                      <option key={cat} value={cat} className="bg-[var(--bg-secondary)]">{cat}</option>
                    ))}
                  </select>
                  <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-[var(--text-muted)]"><i className="fa-solid fa-chevron-down text-xs"></i></div>
                </div>
                ...
                <div className="relative group">
                  <select
                    ...
                    className="w-full bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-2xl p-4 text-sm font-bold outline-none appearance-none hover:bg-[var(--bg-secondary)] focus:border-blue-500 transition-all cursor-pointer text-[var(--text-main)]"
                  >
                    <option value="uncategorized" className="bg-[var(--bg-secondary)]">No Folder</option>
                    {folders.map(f => <option key={f.id} value={f.id} className="bg-[var(--bg-secondary)]">{f.name}</option>)}
                  </select>
                  <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-[var(--text-muted)]"><i className="fa-solid fa-folder text-xs"></i></div>
                </div>
              </div>

              {/* Visibility Toggle */}
              <div className="space-y-3">
                <label className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[0.2em] ml-1">Visibility</label>
                <button
                  type="button"
                  onClick={() => setIsPublic(!isPublic)}
                  className={`w-full py-4 rounded-2xl border font-black text-[10px] tracking-widest transition-all ${isPublic ? "bg-blue-600/10 border-blue-500 text-blue-500" : "bg-[var(--bg-primary)] border-[var(--border-color)] text-[var(--text-muted)]"}`}
                >
              ...
              <div className="pt-4 space-y-3">
                <button
                  ...
                  className="w-full py-5 bg-blue-600 text-white rounded-[1.5rem] font-black text-xs tracking-widest shadow-xl shadow-blue-600/20 hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50 disabled:bg-zinc-800 disabled:shadow-none"
                >
                  {loading
                    ? "CREATING..."
      ...
      <style>{`
        ...
        .editor-custom .ql-editor.ql-blank::before { color: var(--text-muted) !important; left: 2rem !important; font-style: normal; opacity: 0.5; }
        
        /* Adjusting Quill icons based on theme variables */
