...
export function NoteModal({
  ...
  return (
    ...
    >
      ...
      {!hideFolderSelection && (
        <div className="flex flex-col gap-1">
          <label className="text-xs font-bold text-white/40 uppercase">Folder</label>
          <select
      ...
      {/* Content */}
      <div className="flex flex-col gap-1">
        <label className="text-xs font-bold text-white/40 uppercase">Content</label>
        <textarea
          rows="15"
          value={formData.content}
          onChange={(e) => setFormData({ ...formData, content: e.target.value })}
          placeholder="Write your note here..."
          className="w-full px-5 py-5 rounded-xl bg-white/5 border border-white/10 focus:border-(--btn-primary) outline-none text-white/80 leading-relaxed resize-none scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent"
          required
        ></textarea>
      </div>
